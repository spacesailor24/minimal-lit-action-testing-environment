...
import { hexlify, splitSignature, hexZeroPad, joinSignature } from '@ethersproject/bytes'
...
export class Lit {
  ...
  async uploadLitActionToIPFS(code: any): Promise<string> {
    const res = await fetch("https://explorer.litprotocol.com/api/pinata/upload", {
      method: "POST",
      headers: {
  ...
  async mintPKP(delegateeAddress: string) {
    ...
    try {
      ...
      console.log('what that will look like inside the NFT: ', litContracts.utils.getBytesFromMultihash(this.lit_action_ipfs_id))
      ...
      tokenId = mintTxReceipt?.events ? mintTxReceipt?.events[0].topics[1] : undefined
      console.log('Minted token id: ', tokenId)
      pkpPubKey = await litContracts.pkpPermissionsContract.read.getPubkey(tokenId!)
  ...
  async signTX(pkpPubKey: string, pkpTokenId: string, delegateePrivateKey: string, tx: any): Promise<any> {
    ...
    try {
      ...
      const executeJSPayload = {
        ...
        jsParams: {
          ...
          userId: litContracts.utils.getBytesFromMultihash(this.lit_action_ipfs_id),
        },
      }
